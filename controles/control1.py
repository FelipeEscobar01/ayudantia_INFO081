...
def filtra(L: list, ref: str) -> list:
    ...
    for i in L:
        # Pero necesito una forma de contar las vocales de las strings
        # que estoy recorriendo en la ~lista L~ para asi compararlas
        # con aquellas de la ~string de referencia~.
        #
        # | Explicacion mas abajito |
        #
        # Tengo la forma, ahora comparo el numero de vocales que tengo
        # en la string en el indice i de la lista L con el numero que 
        # tengo en la string ref, evaluo si el numero de i es mayor o 
        # igual que el de ref.
        if cuenta_vocales(i) >= cuenta_vocales(ref):
            # ¿ES MAYOR O IGUAL? ... SI
...
def cuenta_vocales(string: str) -> int:
    ...
    # pythonescas, la mostrada es menos.
